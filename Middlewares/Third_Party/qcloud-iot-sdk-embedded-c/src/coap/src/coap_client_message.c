...
static int _coap_client_send_ack(CoAPClient *client, int msgId) {
    ...
    if ((ret = coap_message_type_set(&ack, COAP_MSG_ACK)) != QCLOUD_ERR_SUCCESS)
    {
        coap_message_destroy(&ack);
        IOT_FUNC_EXIT_RC(ret)
    }

    if ((ret = coap_message_id_set(&ack, msgId)) != QCLOUD_ERR_SUCCESS)
    {
        coap_message_destroy(&ack);
        IOT_FUNC_EXIT_RC(ret)
    }
...
static void _coap_message_handle(CoAPClient *client, unsigned char *buf, unsigned short datalen) {
    ...
    if ((rc = deserialize_coap_message(&recv_message, (char*)buf, datalen)) != QCLOUD_ERR_SUCCESS) {
